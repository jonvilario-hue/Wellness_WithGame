...
const TimbreModule = ({ onComplete }: { onComplete: () => void }) => {
    const { engine } = useAudioEngine();
    const assetsToLoad: AssetId[] = useMemo(() => ['instr-piano', 'instr-drum', 'instr-bell'], []);
    const { isLoading } = usePreloadAssets(assetsToLoad);
...
const Menu = ({ onSelectMode, focus }: { onSelectMode: (mode: GaMode) => void, focus: TrainingFocus }) => {
    const { Icon, label } = FOCUS_MODE_META[focus];
    return (
        <div className="flex flex-col items-center gap-4 w-full">
            <div className="flex flex-col items-center gap-2 text-violet-300 mb-4">
...
const modeConfig: Record<GaMode, { title: string, Icon: React.ElementType, Component: React.FC<{ onComplete: () => void }> }> = {
    ...
    timing: { title: "Timing", Icon: Music, Component: () => <GameStub name="Rhythm Reproduction" description="This mode is not yet implemented." chcFactor="Auditory Processing (Ga)" /> },
    timbre: { title: "Timbre", Icon: Ear, Component: TimbreModule },
    recall: { title: "Melody Recall", Icon: Brain, Component: () => <GameStub name="Melody Recall" description="This mode is not yet implemented." chcFactor="Auditory Processing (Ga)" /> },
    segregation: { title: "Segregation", Icon: Bot, Component: () => <GameStub name="Stream Segregation" description="This mode is not yet implemented." chcFactor="Auditory Processing (Ga)" /> },
    localization: { title: "Localization", Icon: Locate, Component: LocalizationModule },
    prosody: { title: "Prosody", Icon: Headphones, Component: () => <GameStub name="Prosody Recognition" description="This mode is not yet implemented." chcFactor="Auditory Processing (Ga)" /> },
};
...
export function AuditoryProcessingRouter() {
    ...
    const currentFocus = (isGlobalFocusLoaded && isOverrideLoaded) ? (override || globalFocus) : 'neutral';
    ...
    const renderActiveMode = () => {
        if (activeMode === 'menu') {
            return <Menu onSelectMode={handleSelectMode} focus={currentFocus} />;
        }
