...
import { ArrowLeft, Brain, Settings, Headphones } from 'lucide-react';
...
import { AuditorySceneSubtraction } from '@/components/training/glr/auditory-scene-subtraction';
import { FOCUS_MODE_META } from '@/lib/mode-constants';

export default function TrainingPage() {
  const domain = 'Glr' as CHCDomain;
  ...
  const GameComponent = effectiveFocus === 'music' ? AuditorySceneSubtraction : (gameComponents[domainInfo.key] || (() => <p>Game not found</p>));
  const gameTitle = effectiveFocus === 'music' ? "Auditory Scene Subtraction" : (domainInfo.gameTitle || domainInfo.name);
  ...
  return (
    <div className="flex flex-col min-h-screen bg-background text-foreground">
      <header className="px-4 sm:px-6 md:px-8 py-4 border-b bg-card sticky top-0 z-10">
       ...
       <div className="border-b bg-card sticky top-[103px] z-10">
